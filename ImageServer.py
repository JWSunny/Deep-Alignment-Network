...
class ImageServer(object):
    ...
    def PrepareData(self, imageDirs, boundingBoxFiles, meanShape, startIdx, nImgs, mirrorFlag):
        filenames = []   ## 此list和filenamesInDir完全一样，可以去除 ( 存储所有的图片数据 jpg和png )
        landmarks = []   ## 此list用于存储特征点坐标   （ 存储所有图片数据对应的关键点坐标 ）
        ...
        for i in range(len(imageDirs)):
            ...
            for j in range(len(filenamesInDir)):
                ...
                landmarks.append(newutils.loadFromPts(ptsFilename))

                if boundingBoxFiles is not None:
                    basename = path.basename(filenamesInDir[j])
                    boundingBoxes.append(boundingBoxDict[basename])
                
		## filenames所有的数据图片名   landmarks所有图片对应的关键点
        ...
        mirrorList = [False for i in range(len(filenames))]
        if mirrorFlag:     
            mirrorList = mirrorList + [True for i in range(len(filenames))]
            filenames = np.concatenate((filenames, filenames))

            landmarks = np.vstack((landmarks, landmarks))    ### 按行方向进行拼接
            ## 修改部分
            if boundingBoxFiles is not None:
    ...
    def LoadImages(self):
        ...
        for i in range(len(self.filenames)):
            ...
            if self.mirrors[i]:
                self.origLandmarks[i] = newutils.mirrorShape(self.origLandmarks[i], img.shape)
                img = np.fliplr(img)
            ...
            if self.initialization == 'rect':
                bestFit = newutils.bestFitRect(groundTruth, self.meanShape)#仅仅把meanshape适应进入由landmark确定的框中
            elif self.initialization == 'similarity':
                bestFit = newutils.bestFit(groundTruth, self.meanShape)#找到meanShape到gt的最优变换，并变换之
            elif self.initialization == 'box':
                bestFit = newutils.bestFitRect(groundTruth, self.meanShape, box=self.boundingBoxes[i])#仅仅把meanshape适应进入由检测到的bbx确定的框中
    ...
    def CropResizeRotateAll(self):
        ...
        for i in range(self.initLandmarks.shape[0]):
            ...
            newImgs.append(tempImg.transpose((1,2,0)))  ## 转置操作
            newInitLandmarks.append(tempInit)
            newGtLandmarks.append(tempGroundTruth)
    ...
    def CropResizeRotate(self, img, initShape, groundTruth):
        ...
        A, t = newutils.bestFit(destShape, initShape, True)
