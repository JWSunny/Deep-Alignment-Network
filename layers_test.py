...
def TransformParamsLayer(SrcShapes, DstShape):
    ...
    def bestFit(src, dst):
        ...
        destMean = tf.reduce_mean(destination, axis=0)  ## axis=0 表示求每一列的均值
        ...
        srcMean = tf.tensordot(srcMean, A, 1)  ## 相当于两个矩阵相乘 srcMean * A <--------> 得到的矩阵维度是 (N, 2)
        # import pdb;pdb.set_trace()
        return  tf.concat((tf.reshape(A, (-1,)), destMean - srcMean), 0)  ###  返回的是 N * 6 维的矩阵
...
#     src=SrcShapes
#     dst=DstShape
#     source = tf.reshape(src, (-1, N_LANDMARK, 2))
#     destination = tf.reshape(dst, (-1, N_LANDMARK, 2))
#     destMean = tf.reduce_mean(destination, axis=1, keep_dims=True)  ## axis=0 表示求每一列的均值
#     srcMean = tf.reduce_mean(source, axis=1, keep_dims=True)
#     srcCenter = source - srcMean
#     dstCenter = destination - destMean
#
#     dot_result = tf.reduce_sum(tf.multiply(srcCenter, dstCenter),axis=[1, 2])
#     norm_pow_2 = tf.pow(tf.norm(srcCenter, axis=[1, 2]), 2)
#
#     a = dot_result / norm_pow_2
#     b = tf.reduce_sum(tf.multiply(srcCenter[:, :, 0], dstCenter[:, :, 1]) - tf.multiply(srcCenter[:, :, 1],dstCenter[:, :, 0]),1) / norm_pow_2
#
#     r = tf.reshape(tf.stack([a, b, -b, a], axis=1), (-1, 2, 2))
#     t = tf.reshape((destMean - tf.matmul(srcMean, r)),(-1, 2))
#     r = tf.reshape(r, (-1, 2 * 2))
#     return  tf.concat((r, t), 1)  ###  返回的是 N * 6 维的矩阵



def AffineTransformLayer(Image, Param, Inverse=False):
    '''
...
HalfSize = 8
Offsets = tf.constant(np.array(list(itertools.product(range(-HalfSize, HalfSize),range(-HalfSize, HalfSize))), dtype=np.int32), shape=(16, 16, 2))
